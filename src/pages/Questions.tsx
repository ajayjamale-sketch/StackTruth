...
export default function Questions() {
  ...
  const filtered = allQuestions.filter(q => {
    const matchSearch = !search || q.title.toLowerCase().includes(search.toLowerCase()) || q.tags.some(t => t.includes(search.toLowerCase()));
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-muted-foreground text-sm mt-1">{allQuestions.length} questions · Ask anything technical</p>
            </div>
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            <button
              ...
              className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-colors ${!selectedTag ? 'bg-primary text-white' : 'bg-muted text-muted-foreground hover:text-foreground'}`}
            >
              All
            </button>
            {allTags.map(tag => (
              <button
                ...
                className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-colors ${selectedTag === tag ? 'bg-primary/20 text-primary border border-primary/30' : 'bg-muted/50 text-muted-foreground hover:bg-muted'}`}
              >
                #{tag}
              </button>
            ))}
            ...
            <div className="bg-card border border-border rounded-xl p-4">
              ...
              <div className="flex flex-wrap gap-1.5">
                {allTags.slice(0, 8).map(tag => (
                  <button key={tag} onClick={() => setSelectedTag(tag)} className="px-2 py-1 bg-muted text-xs text-muted-foreground rounded-lg hover:bg-primary/10 hover:text-primary transition-colors">
                    #{tag}
                  </button>
                ))}
              </div>
