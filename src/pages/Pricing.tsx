    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            <button
              ...
              className={`w-12 h-6 rounded-full transition-colors relative ${yearly ? 'bg-primary' : 'bg-white/20'}`}
            >
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-3 gap-6">
            {pricingPlans.map(plan => (
              <div key={plan.id} className={`relative bg-card border rounded-2xl p-6 flex flex-col ${plan.highlighted ? 'border-primary shadow-2xl shadow-primary/20 scale-105' : 'border-border'}`}>
                {plan.badge && (
                  <div className={`absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 text-xs font-bold rounded-full ${plan.highlighted ? 'bg-primary text-white' : 'bg-muted text-muted-foreground border border-border'}`}>
                    {plan.badge}
                  </div>
                )}
                <div className="mb-5">
                  ...
                  {yearly && plan.price.monthly > 0 && (
                    <p className="text-xs text-accent mt-1">You save ${(plan.price.monthly - plan.price.yearly) * 12}/year</p>
                  )}
                </div>
          ...
          <div className="mt-8 bg-gradient-to-r from-primary/10 to-accent/10 border border-primary/20 rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              ...
              <p className="text-muted-foreground text-sm">Custom plans for large engineering orgs with SSO, dedicated support, and SLA.</p>
            </div>
      ...
      <section className="py-16 bg-muted/30">
        <div className="max-w-2xl mx-auto px-4">
          ...
          <div className="space-y-3">
            {[
              { q: 'Can I change plans at any time?', a: 'Yes, you can upgrade or downgrade your plan at any time. Changes take effect immediately and are prorated.' },
              { q: 'Is there a free trial?', a: 'All paid plans come with a 14-day free trial. No credit card required to start.' },
              { q: 'What payment methods do you accept?', a: 'We accept all major credit cards, PayPal, and for enterprise plans, bank transfer and invoicing.' },
            ].map((faq, i) => (
      ...
      <section className="py-16 bg-background">
        <div className="max-w-2xl mx-auto px-4 text-center">
          ...
          <p className="text-muted-foreground mb-6">Join 180K+ developers. No credit card required.</p>
