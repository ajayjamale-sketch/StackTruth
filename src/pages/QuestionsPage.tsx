...
import { Search, Filter, ThumbsUp, MessageSquare, Eye, CheckCircle, TrendingUp, Clock, Flame, Plus, Tag, Award, BookmarkPlus, ChevronUp, ChevronDown, Sparkles, Shield, Users, MessageCircle, ChevronRight } from "lucide-react";
...
function QuestionCard({ question }: { question: Question }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-6 sm:p-8 rounded-xl hover:shadow-[0_40px_100px_rgba(0,0,0,0.06)] dark:hover:shadow-primary/5 transition-all duration-500 group relative overflow-hidden">
      <div className="flex flex-col sm:flex-row items-start gap-6 sm:gap-10">
        {/* Vote Counter - Ultra Clean */}
        <div className="flex sm:flex-col items-center gap-4 sm:gap-3 flex-shrink-0 bg-slate-50 dark:bg-slate-800/40 p-3 sm:p-4 rounded-xl border border-slate-100 dark:border-slate-800 transition-all duration-500 group-hover:border-primary/20 group-hover:bg-primary/5 w-fit sm:w-16 justify-center">
          <button 
        ...
        <div className="flex-1 min-w-0 space-y-4 sm:space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            <Link to={`/questions/${question.id}`} className="text-xl sm:text-2xl font-black text-slate-950 dark:text-white hover:text-primary transition-colors leading-[1.1] tracking-tighter line-clamp-2">
              {question.title}
            </Link>
            {question.bounty && (
              <span className="inline-block sm:flex-shrink-0 bg-amber-500 text-white text-[9px] font-black px-4 py-1.5 rounded-sm uppercase tracking-widest shadow-lg shadow-amber-500/20 w-fit">
                +{question.bounty} Bounty
              </span>
            )}
          </div>
          
          <p className="text-sm sm:text-base text-slate-500 dark:text-slate-400 leading-relaxed line-clamp-2 font-medium tracking-tight">{question.body}</p>

          <div className="flex flex-wrap gap-2">
            {question.tags.map((tag) => (
              <span key={tag} className="px-2.5 py-1 bg-slate-100 dark:bg-slate-800 text-[9px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 rounded-md border border-slate-100 dark:border-slate-800 group-hover:border-primary/20 transition-all">{tag}</span>
            ))}
          </div>

          <div className="flex flex-col md:flex-row md:items-center justify-between pt-6 border-t border-slate-50 dark:border-slate-800 mt-4 gap-4">
            <div className="flex items-center gap-4 sm:gap-6 text-[9px] font-black text-slate-400 uppercase tracking-widest">
              <span className="flex items-center gap-2"><Eye className="w-3.5 h-3.5 text-primary" />{question.views.toLocaleString()}</span>
              <span className={`flex items-center gap-2 ${question.isAnswered ? 'text-primary' : ''}`}>
                <MessageSquare className="w-3.5 h-3.5" /> {question.answers}
              </span>
            </div>
            <div className="flex items-center gap-3">
              <div className="text-right">
                <p className="text-xs font-black text-slate-900 dark:text-slate-200 leading-none">@{question.author.username}</p>
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">{timeAgo(question.createdAt)}</p>
              </div>
              <div className="w-8 h-8 sm:w-10 sm:h-10 bg-primary/10 dark:bg-primary/20 rounded-lg flex items-center justify-center text-primary font-black text-xs sm:text-sm border border-primary/10">
      ...
      <section className="relative bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-8 sm:p-12 lg:p-16 rounded-xl shadow-2xl overflow-hidden group">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-12 text-center lg:text-left">
          <div className="space-y-6 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-primary/5 text-primary rounded-sm text-[10px] font-black uppercase tracking-[0.2em] border border-primary/10 mx-auto lg:mx-0">
              <MessageCircle className="w-4 h-4" /> Global Discussion Hub
            </div>
            <h1 className="text-4xl sm:text-5xl md:text-7xl font-black text-slate-950 dark:text-white tracking-tighter leading-none">The Technical <br /> <span className="text-primary">Audits</span></h1>
            <p className="text-lg sm:text-xl text-slate-500 dark:text-slate-400 font-medium leading-relaxed tracking-tight">Explore community-verified solutions and architectural audits for complex engineering challenges.</p>
          </div>
          <div className="flex flex-col items-center lg:items-end gap-6 w-full lg:w-auto">
            <div className="flex gap-4">
               <div className="bg-slate-100/50 dark:bg-slate-800/40 p-4 sm:p-6 rounded-xl border border-slate-100 dark:border-slate-800 text-center w-28 sm:w-32">
                 <p className="text-2xl sm:text-3xl font-black text-slate-950 dark:text-white">{MOCK_QUESTIONS.length}</p>
                 <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Total Audits</p>
               </div>
            </div>
            {isAuthenticated && (
              <button onClick={() => setShowAsk(true)} className="w-full bg-primary text-white font-black px-8 py-4 sm:px-10 sm:py-5 rounded-lg flex items-center justify-center gap-3 hover:opacity-90 transition-all shadow-2xl active:scale-95 text-xs sm:text-sm">
                <Plus className="w-5 h-5 sm:w-6 sm:h-6" /> Initiate New Audit
              </button>
            )}
          </div>
        </div>
        ...
        <div className="lg:col-span-8 space-y-12">
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-6 w-full">
              {/* Filter Tabs */}
              <div className="flex gap-2 p-1.5 bg-slate-100 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 overflow-x-auto no-scrollbar shadow-inner w-full max-w-full md:w-auto">
                {FILTERS.map((f) => (
                  <button
                    key={f}
                    onClick={() => setFilter(f)}
                    className={`px-6 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap active:scale-95 flex-shrink-0 ${filter === f ? 'bg-white dark:bg-slate-900 text-primary shadow-md' : 'text-slate-500 hover:text-primary'}`}
                  >
                    {f}
                  </button>
                ))}
              ...
              <div className="flex gap-2 p-1.5 bg-slate-100 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 overflow-x-auto no-scrollbar shadow-inner w-full max-w-full md:w-auto">
                {SORT_OPTIONS.map((opt) => (
                  <button
                    ...
                    className={`flex items-center gap-2 px-6 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap active:scale-95 flex-shrink-0 ${sort === opt.label ? 'bg-white dark:bg-slate-900 text-primary shadow-md' : 'text-slate-500 hover:text-primary'}`}
                  >
