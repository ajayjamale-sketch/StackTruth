...
interface Answer {
  id: string;
  content: string;
  author: { name: string; avatar: string; reputation: number; badge: string };
  votes: number;
  accepted: boolean;
  createdAt: string;
}
...
export default function QuestionDetail() {
  ...
  const initialAnswers = answers.length === 0 ? mockAnswers : answers;
  const displayAnswers = answers.length === 0 ? mockAnswers : answers;
  ...
  const handleShare = (type: 'question' | 'answer', answerId?: string) => {
    const url = window.location.href;
    if (type === 'answer' && answerId) {
      copyToClipboard(`${url}#answer-${answerId}`, 'Answer link copied!');
    } else {
      copyToClipboard(url, 'Question link copied!');
    }
  };
  ...
  const handleFlag = () => {
    ...
    if (reason && reason.trim()) {
      toast.success('Thank you for the report. Our moderators will review it.');
    } else if (reason !== null) {
      toast.error('Please enter a reason.');
    }
  };

  // Markdown toolbar – insert at cursor or append
  ...
  const handleAnswer = () => {
    ...
    const newAnswer: Answer = {
      ...
      author: {
        name: 'You',
        avatar: 'https://ui-avatars.com/api/?background=2563eb&color=fff&name=You',
        reputation: 1250,
        badge: 'Member',
      },
      votes: 0,
  ...
  // For demo we assume current user is the author (mock check)
  const handleAcceptAnswer = (answerId: string) => {
    if (acceptedAnswerId === answerId) {
      // Unaccept (optional)
      setAcceptedAnswerId(null);
      toast.info('Answer unaccepted');
    } else {
      setAcceptedAnswerId(answerId);
      toast.success('Answer accepted!');
    }
    // Update answers' accepted flag
    setAnswers(prev =>
      prev.map(ans => ({
        ...ans,
        accepted: ans.id === answerId,
      }))
    );
  };

  // Determine if current user can accept answers (question author)
  // We'll assume current user is the one who asked the question (mock)
  const isQuestionAuthor = true; // In real app, compare with logged-in user
        ...
        <div className="flex gap-6 mb-8">
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground mb-5">
              ...
              <span className="flex items-center gap-1"><MessageSquare className="w-3.5 h-3.5" />{displayAnswers.length} answers</span>
            </div>
...
client.release(); // BUG: not in finally!`}</pre>
        ...
        <div className="border-t border-border pt-8 mb-8">
          <h2 className="text-xl font-bold mb-6">{displayAnswers.length} Answers</h2>
          <div className="space-y-6">
            {displayAnswers.map(answer => {
              const isAccepted = answer.accepted || acceptedAnswerId === answer.id;
              return (
                <div
                  key={answer.id}
                  id={`answer-${answer.id}`}
                  className={`flex gap-6 pb-6 border-b border-border last:border-0 ${isAccepted ? 'bg-accent/5 -mx-4 px-4 rounded-xl border border-accent/20' : ''}`}
                >
                  <div className="flex flex-col items-center gap-2 flex-shrink-0">
                    <button
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <button
                          onClick={() => handleShare('answer', answer.id)}
          ...
          <div className="bg-card border border-border rounded-xl p-4 mb-3">
            <div className="flex gap-2 mb-2 border-b border-border pb-2">
              {['B', 'I', 'Code', 'Link', 'Quote'].map(f => {
                const markdownMap: Record<string, [string, string]> = {
                  B: ['**', '**'],
                  I: ['*', '*'],
                  Code: ['`', '`'],
                  Link: ['[', '](url)'],
                ...
                const [start, end] = markdownMap[f] || ['', ''];
                return (
                  <button
                    key={f}
                    onClick={() => insertMarkdown(start, end)}
                    className="text-xs px-2 py-1 rounded text-muted-foreground hover:bg-muted transition-colors font-mono"
                    type="button"
                  >
                    {f}
                  </button>
...
// Mock answers data (moved here to avoid outer scope)
