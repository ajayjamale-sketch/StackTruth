...
const perks = [
  ...
  'AI-powered code review on every submission',
  'Access to 2.4M+ technical solutions',
  'Community of 180K+ expert engineers',
];
      ...
      <div className="flex-1 flex flex-col">
        <div className="flex items-center justify-between p-6">
          ...
          <div className="flex items-center gap-3 ml-auto">
            <button onClick={toggleTheme} className="w-8 h-8 rounded-lg flex items-center justify-center text-muted-foreground hover:bg-muted transition-colors">
        ...
        <div className="flex-1 flex items-center justify-center px-6 py-8">
          <div className="w-full max-w-md">
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              <Button variant="outline" className="w-full" disabled={loading} onClick={() => { login(); navigate('/dashboard'); }}>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input id="name" placeholder="Alex Chen" {...register('name')} className={errors.name ? 'border-destructive' : ''} />
                  {errors.name && <p className="text-xs text-destructive mt-1">{errors.name.message}</p>}
                </div>
                <div>
                  <Label htmlFor="username" className="text-sm font-medium mb-1.5 block">Username</Label>
                  <Input id="username" placeholder="alexchen" {...register('username')} className={errors.username ? 'border-destructive' : ''} />
              ...
              <div>
                ...
                <div className="relative">
                  <Input id="password" type={showPwd ? 'text' : 'password'} placeholder="••••••••" {...register('password')} className={errors.password ? 'border-destructive pr-10' : 'pr-10'} />
                ...
                {password && (
                  <div className="flex gap-2 mt-2">
                    {strengthChecks.map(c => (
                      <div key={c.label} className={`flex-1 h-1 rounded-full transition-colors ${c.passed ? 'bg-accent' : 'bg-border'}`} />
                    ))}
                  </div>
                )}
              ...
              <Button type="submit" className="w-full bg-primary hover:bg-primary/90 h-11" disabled={loading}>
                {loading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Creating account...</> : <>Create Free Account <ArrowRight className="w-4 h-4 ml-2" /></>}
              </Button>
            </form>
