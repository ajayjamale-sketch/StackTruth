...
import { ArrowUpCircle, MessageSquare, CheckCircle, Clock, Tag, ArrowRight } from 'lucide-react';
    ...
    <section className="py-24 bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div>
            <Badge variant="outline" className="mb-4 border-primary/30 text-primary">Technical Q&A</Badge>
            ...
            <ul className="space-y-3 mb-8">
              {['Rich markdown editor with code block support', 'AI-powered answer quality scoring', 'Expert badges and reputation verification', 'Tag-based discovery and smart search'].map(item => (
            ...
            <div className="flex flex-wrap gap-3">
              <Button asChild className="bg-primary hover:bg-primary/90">
                <Link to="/questions">Browse Questions <ArrowRight className="w-4 h-4 ml-2" /></Link>
              </Button>
          ...
          <div className="space-y-3">
            {questions.map((q, i) => (
              <Link key={i} to="/questions" className="block bg-card border border-border rounded-xl p-4 hover:border-primary/30 transition-all card-hover cursor-pointer">
                <div className="flex gap-4">
                  <div className="flex-shrink-0 text-center">
                    <div className={`text-lg font-bold ${q.accepted ? 'text-accent' : 'text-muted-foreground'}`}>{q.votes}</div>
                    <div className="text-xs text-muted-foreground">votes</div>
                    <div className={`mt-1 px-2 py-0.5 rounded text-xs font-medium ${q.accepted ? 'bg-accent/10 text-accent' : 'bg-muted text-muted-foreground'}`}>
                      {q.answers} ans
                    </div>
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="text-sm font-medium text-foreground mb-2 leading-snug line-clamp-2">{q.title}</h4>
            ...
            <Link to="/questions" className="flex items-center justify-center py-3 text-sm text-primary hover:underline gap-1">
