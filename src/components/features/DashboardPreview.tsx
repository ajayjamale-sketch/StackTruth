    ...
    <section className="py-24 section-dark relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-12">
          ...
          <p className="text-slate-400 max-w-xl mx-auto">Track your growth, manage questions, run code reviews, and collaborate with your team — all from a single beautiful dashboard.</p>
        </div>
        ...
        <div className="bg-[#0a0f1e] border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
          ...
          <div className="flex items-center justify-between px-5 py-3 border-b border-white/10">
            <div className="flex items-center gap-3">
              ...
              <span className="text-white/40 text-xs">StackTruth Dashboard</span>
            </div>
            <div className="flex gap-2">
              {['Overview', 'Questions', 'Reviews', 'AI'].map(t => (
                <button key={t} className={`text-xs px-2.5 py-1 rounded-lg ${t === 'Overview' ? 'bg-blue-500/20 text-blue-400' : 'text-white/40 hover:text-white/70'}`}>{t}</button>
              ))}
            </div>
          </div>
          ...
          <div className="flex">
            {/* Sidebar */}
            <div className="w-40 border-r border-white/10 p-3 space-y-1 hidden md:block flex-shrink-0">
              {[
                ...
                return (
                  <div key={item.label} className={`flex items-center gap-2 px-2.5 py-2 rounded-lg text-xs ${item.active ? 'bg-blue-500/20 text-blue-400' : 'text-white/40'}`}>
            ...
            <div className="flex-1 p-4">
              ...
              <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                <p className="text-white/60 text-xs font-semibold mb-3">Recent Questions</p>
                {[
                  ...
                  <div key={i} className="flex items-center gap-2 py-2 border-b border-white/5 last:border-0">
                    ...
                    <p className="text-white/60 text-xs truncate">{q}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="text-center mt-8">
          <Button asChild className="bg-primary hover:bg-primary/90 btn-glow">
            <Link to="/register">Access Your Dashboard <ArrowRight className="w-4 h-4 ml-2" /></Link>
          </Button>
        </div>
      </div>
    </section>
