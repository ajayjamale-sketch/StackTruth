...
import { ArrowRight, MessageSquare, User, Copy, ThumbsUp } from 'lucide-react';
    ...
    <section className="py-24 bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div>
            <Badge variant="outline" className="mb-4 border-primary/30 text-primary">Developer Tools</Badge>
            ...
            <div className="flex flex-wrap gap-2 mb-8">
              {['Code Review', 'Technical Q&A', 'Architecture Patterns', 'Best Practices', 'Debugging Tips', 'Performance Insights'].map(cap => (
                <Badge key={cap} variant="secondary" className="text-xs">{cap}</Badge>
              ))}
            </div>
            <Button asChild className="bg-primary hover:bg-primary/90">
              <Link to="/questions">Explore Questions <ArrowRight className="w-4 h-4 ml-2" /></Link>
            </Button>
          </div>
          ...
          <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-xl">
            <div className="flex items-center gap-3 px-5 py-4 border-b border-border">
              ...
              <Badge className="ml-auto text-xs bg-accent/10 text-accent border-accent/20">Verified</Badge>
            </div>
              ...
              {messages.map((msg, i) => (
                <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${
                    ...
                    {msg.role === 'community' ? <MessageSquare className="w-3.5 h-3.5 text-primary" /> : <User className="w-3.5 h-3.5 text-muted-foreground" />}
                  </div>
